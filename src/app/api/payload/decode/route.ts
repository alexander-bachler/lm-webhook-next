...
function executeDecoder(decoderCode: string, payload: string, fPort?: number) {
  try {
    ...
    // Erstelle Input-Objekt für Decoder
    const input = {
    ...
    const context = {
      ...
      Buffer,
      console: {
        log: (...args: any[]) => console.log('Decoder Log:', ...args),
        error: (...args: any[]) => console.error('Decoder Error:', ...args),
        warn: (...args: any[]) => console.warn('Decoder Warning:', ...args)
      },
    ...
    // Kompiliere und führe Decoder-Code aus
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Bestimme Decoder basierend auf Device-Informationen, Device-Type oder Payload
    if (decoder === 'auto') {
      // Verwende Device-Informationen falls verfügbar
      if (deviceInfo?.decoder && deviceInfo.decoder !== 'auto') {
        ...
        // Versuche automatische Erkennung basierend auf Payload
        const bytes = Buffer.from(payload, 'hex');
        if (bytes[0] === bytes.length - 1 && bytes[1] === 0x44) {
          usedDecoder = 'integra-topas-sonic';
    ...
    // Führe entsprechende Dekodierung durch
    switch (usedDecoder) {
      ...
      case 'raw':
        ...
        break;
      default:
        // Versuche Decoder-Datei zu laden
        const decoderCode = await loadDecoder(usedDecoder);
        if (decoderCode) {
          decodedData = executeDecoder(decoderCode, payload, fPort);
        } else {
          decodedData = {
            data: {
              raw_hex: payload,
