// @ts-nocheck
import { loadDeviceBundle, persistDeviceBundle } from './device-store';
const fs = require('fs');
...
class DeviceManager {
  ...
  loadDevices() {
    try {
      const bundle = loadDeviceBundle();
      return {
        devices: bundle.devices,
        decoders: bundle.decoders || {},
        lastUpdate: bundle.lastUpdate
      };
    } catch (error) {
  ...
  saveDevices() {
    try {
      persistDeviceBundle({
        ...
        decoders: this.devices.decoders || {},
        lastUpdate: this.devices.lastUpdate
      });
      logger.info('Devices erfolgreich gespeichert (SQLite)');
    } catch (error) {
  ...
  registerDevice(deviceData) {
    ...
    const device = {
      ...
      dataCount: 0,
      metadata: {},
      lineMetrics: lineMetrics || { enabled: false, dataPoints: {} },
      ...(outputs ? { outputs } : {}),
      ...(schedules ? { schedules } : {}),
    };
  ...
  deleteDevice(deviceId) {
    ...
    this.devices.devices[deviceId].archived = true;
    this.devices.devices[deviceId].updatedAt = new Date().toISOString();
    this.saveDevices();
    logger.info('Device archiviert:', { deviceId });
  }
  ...
  getDeviceByEUI(deviceEUI) {
    return (
      Object.values(this.devices.devices).find(
        (device) => device.deviceEUI === deviceEUI && !device.archived
      ) || null
    );
  }
...
export default deviceManagerSingleton; 